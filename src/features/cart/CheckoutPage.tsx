...
const checkoutSchema = z.object({
  ...
  cvv: z.string().regex(/^\d{3,4}$/, "El CVV debe tener 3-4 dígitos"),
});
...
export const CheckoutPage = () => {
  ...
  const onSubmit = async (data: CheckoutForm) => {
    ...
    try {
      ...
      clearCart();
      setIsSuccess(true);
      addToast("¡Pedido realizado exitosamente!", "success");
    } catch (error) {
      console.error("Checkout error:", error);
      addToast("Error al realizar el pedido. Por favor, intenta de nuevo.", "error");
    } finally {
      ...
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        <div className="bg-white rounded-lg shadow-sm p-8">
          ...
          <p className="text-gray-500 mb-8">
            Gracias por tu compra. Tu pedido ha sido realizado exitosamente y será enviado pronto.
          </p>
          <Link
            to="/"
  ...
  if (items.length === 0 && !isSuccess) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <p className="text-gray-500 mb-6">Agrega algunas mascotas antes de pagar.</p>
          ...
          <div className="ml-3">
            {user ? (
              <p className="text-sm text-blue-700">
                Conectado como <span className="font-medium">{user}</span>. Hemos prellenado tu información.
              </p>
            ) : (
              <p className="text-sm text-blue-700">
                Pagar como invitado o <Link to="/users/login" className="font-medium underline hover:text-blue-600">iniciar sesión</Link> para guardar tu pedido en tu cuenta.
              </p>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-100">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="pt-6 border-t border-gray-200">
              <h2 className="text-xl font-semibold text-gray-900 mb-4">Detalles de pago</h2>
              <div className="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
                <div className="sm:col-span-2">
                ...
                <div>
                  <label htmlFor="expiry" className="block text-sm font-medium text-gray-700 mb-1">Fecha de vencimiento</label>
                  <input
                    id="expiry"
                    {...register('expiry')}
                    placeholder="MM/AA"
